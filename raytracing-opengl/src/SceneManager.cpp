...
SceneManager::SceneManager(int wind_width, int wind_height,
                           scene_container* scene, GLWrapper* wrapper) {
...
void SceneManager::init() {
    ...
    auto mouseFunc = [](GLFWwindow* w, double x, double y) {
        static_cast<SceneManager*>(glfwGetWindowUserPointer(w))
            ->glfw_mouse_callback(w, x, y);
    };
    auto keyFunc = [](GLFWwindow* w, int a, int b, int c, int d) {
        static_cast<SceneManager*>(glfwGetWindowUserPointer(w))
            ->glfw_key_callback(w, a, b, c, d);
    };
    ...
    glfwSetFramebufferSizeCallback(wrapper->window,
...
void SceneManager::update_scene(float deltaTime) {
    ...
    scene->scene.quat_camera_rotation =
        glm::quat(glm::vec3(glm::radians(-pitch), glm::radians(yaw), 0));
...
void SceneManager::glfw_key_callback(GLFWwindow* window, int key, int scancode,
                                     int action, int mods) {
...
void SceneManager::glfw_framebuffer_size_callback(GLFWwindow* wind, int width,
                                                  int height) {
...
void SceneManager::glfw_mouse_callback(GLFWwindow* window, double xpos,
                                       double ypos) {
...
rt_material SceneManager::create_material(glm::vec3 color, int specular,
                                          float reflect, float refract,
                                          glm::vec3 absorb, float diffuse,
                                          float kd, float ks) {
...
rt_sphere SceneManager::create_sphere(glm::vec3 center, float radius,
                                      rt_material material, bool hollow) {
    rt_sphere sphere = {};
...
rt_plane SceneManager::create_plane(glm::vec3 normal, glm::vec3 pos,
                                    rt_material material) {
    rt_plane plane = {};
    plane.normal = normal;
    plane.pos = pos;
...
rt_box SceneManager::create_box(glm::vec3 pos, glm::vec3 form,
                                rt_material material) {
    rt_box box = {};
    box.form = form;
    box.pos = pos;
...
rt_torus SceneManager::create_torus(glm::vec3 pos, glm::vec2 form,
                                    rt_material material) {
    rt_torus torus = {};
    torus.form = form;
    torus.pos = pos;
...
rt_ring SceneManager::create_ring(glm::vec3 pos, float r1, float r2,
                                  rt_material material) {
    rt_ring ring = {};
    ring.pos = pos;
    ring.mat = material;
...
rt_light_point SceneManager::create_light_point(glm::vec4 position,
                                                glm::vec3 color,
                                                float intensity, float linear_k,
                                                float quadratic_k) {
...
rt_light_direct SceneManager::create_light_direct(glm::vec3 direction,
                                                  glm::vec3 color,
                                                  float intensity) {
...
template <typename T>
void SceneManager::init_buffer(GLuint* ubo, const char* name, int bindingPoint,
                               std::vector<T>& v) {
    wrapper->init_buffer(ubo, name, bindingPoint, sizeof(T) * v.size(),
                         v.data());
}
