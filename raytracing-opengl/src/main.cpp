...
namespace update {
    int jupiter = -1, saturn = -1, saturn_rings = -1, mars = -1, box = -1,
        torus = -1;
}
...
int main() {
    ...
    // lights
    scene.lights_point.push_back(
        SceneManager::create_light_point({ 3, 5, 0, 0.1 }, { 1, 1, 1 }, 25.5));
    scene.lights_direct.push_back(
        SceneManager::create_light_direct({ 3, -1, 1 }, { 1, 1, 1 }, 1.5));

    // blue sphere
    scene.spheres.push_back(SceneManager::create_sphere(
        { 2, 0, 6 }, 1, SceneManager::create_material({ 0, 0, 1 }, 50, 0.35)));
    // red sphere
    scene.spheres.push_back(SceneManager::create_sphere(
        { -1, 0, 6 }, 1, SceneManager::create_material({ 1, 0, 0 }, 100, 0.1),
        true));
    // transparent sphere
    scene.spheres.push_back(SceneManager::create_sphere(
        { 0.5, 2, 6 }, 1,
        SceneManager::create_material({ 1, 1, 1 }, 200, 0.1, 1.125, { 1, 0, 2 },
                                      1),
        true));

    // jupiter
    rt_sphere jupiter = SceneManager::create_sphere(
        {}, 5000, SceneManager::create_material({}, 0, 0.0f));
    ...
    rt_sphere saturn = SceneManager::create_sphere(
        {}, saturnRadius, SceneManager::create_material({}, 0, 0.0f));
    saturn.textureNum = 2;
    ...
    // mars
    rt_sphere mars = SceneManager::create_sphere(
        {}, 500, SceneManager::create_material({}, 0, 0.0f));
    mars.textureNum = 3;
    ...
    // ring
    {
        rt_ring ring = SceneManager::create_ring(
            {}, saturnRadius * 1.1166, saturnRadius * 2.35,
    ...
    // floor
    scene.boxes.push_back(SceneManager::create_box(
        { 0, -1.2, 6 }, { 10, 0.2, 5 },
        SceneManager::create_material({ 1, 0.6, 0 }, 100, 0.05)));
    // box
    rt_box box = SceneManager::create_box(
        { 8, 1, 6 }, { 1, 1, 1 },
    ...
    // torus
    rt_torus torus = SceneManager::create_torus(
        { -9, 0.5, 6 }, { 1.0, 0.5 },
    ...
    // cone
    rt_material coneMaterial = SceneManager::create_material(
        { 234 / 255.0f, 17 / 255.0f, 82 / 255.0f }, 200, 0.2);
    rt_surface cone =
        SurfaceFactory::GetEllipticCone(1 / 3.0f, 1 / 3.0f, 1, coneMaterial);
    cone.pos = { -5, 4, 6 };
    ...
    // cylinder
    rt_material cylinderMaterial = SceneManager::create_material(
        { 200 / 255.0f, 255 / 255.0f, 0 / 255.0f }, 200, 0.2);
    rt_surface cylinder = SurfaceFactory::GetEllipticCylinder(
        1 / 2.0f, 1 / 2.0f, cylinderMaterial);
    cylinder.pos = { 5, 0, 6 };
    ...
    auto jupiterTex =
        glWrapper.load_texture(1, "8k_jupiter.jpg", "texture_sphere_1");
    auto saturnTex =
        glWrapper.load_texture(2, "8k_saturn.jpg", "texture_sphere_2");
    auto marsTex = glWrapper.load_texture(3, "2k_mars.jpg", "texture_sphere_3");
    auto ringTex =
        glWrapper.load_texture(4, "8k_saturn_ring_alpha.png", "texture_ring");
...
void update_scene(scene_container& scene, float deltaTime, float time) {
    if (update::jupiter != -1) {
        ...
        jupiter->quat_rotation *=
            glm::angleAxis(deltaTime / 15, glm::vec3(0, 1, 0));
    }
    ...
    if (update::mars != -1) {
        ...
        mars->quat_rotation *=
            glm::angleAxis(deltaTime / 5, glm::vec3(0, 1, 0));
    }

    if (update::box != -1) {
        rt_box* box = &scene.boxes[update::box];
        glm::quat q =
            glm::angleAxis(deltaTime, glm::vec3(0.5774, 0.5774, 0.5774));
        box->quat_rotation *= q;
    }
