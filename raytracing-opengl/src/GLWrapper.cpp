...
bool GLWrapper::init_window() {
    ...
    window = glfwCreateWindow(width, height, "RayTracing",
    ...
    float quadVertices[] = { -1.0f, -1.0f, 0.0f,  0.0f, 1.0f, -1.0f,
...
void GLWrapper::gen_framebuffer(GLuint* fbo, GLuint* fboTex,
                                GLenum internalFormat, GLenum format) const {
...
void GLWrapper::init_shaders(rt_defines& defines) {
    const std::string vertexShaderSrc =
        readStringFromFile("./assets/shaders/quad.vert");
    std::string fragmentShaderSrc =
        readStringFromFile("./assets/shaders/rt.frag");

    replace(fragmentShaderSrc, "{SPHERE_SIZE}",
            std::to_string(defines.sphere_size));
    replace(fragmentShaderSrc, "{PLANE_SIZE}",
            std::to_string(defines.plane_size));
    replace(fragmentShaderSrc, "{SURFACE_SIZE}",
            std::to_string(defines.surface_size));
    replace(fragmentShaderSrc, "{BOX_SIZE}", std::to_string(defines.box_size));
    replace(fragmentShaderSrc, "{TORUS_SIZE}",
            std::to_string(defines.torus_size));
    replace(fragmentShaderSrc, "{RING_SIZE}",
            std::to_string(defines.ring_size));
    replace(fragmentShaderSrc, "{LIGHT_POINT_SIZE}",
            std::to_string(defines.light_point_size));
    replace(fragmentShaderSrc, "{LIGHT_DIRECT_SIZE}",
            std::to_string(defines.light_direct_size));
    replace(fragmentShaderSrc, "{ITERATIONS}",
            std::to_string(defines.iterations));
    replace(fragmentShaderSrc, "{AMBIENT_COLOR}",
            to_string(defines.ambient_color));
    replace(fragmentShaderSrc, "{SHADOW_AMBIENT}",
            to_string(defines.shadow_ambient));
...
unsigned int GLWrapper::load_cubemap(std::vector<std::string> faces,
                                     bool genMipmap) {
...
GLuint GLWrapper::load_texture(int texNum, const char* name,
                               const char* uniformName, GLuint wrapMode) {
...
void GLWrapper::init_buffer(GLuint* ubo, const char* name, int bindingPoint,
